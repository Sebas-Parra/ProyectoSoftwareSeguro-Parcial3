<!-- components/dialogs/ModuleFormDialog.vue -->
<template>
  <Dialog 
    :visible="visible" 
    :header="isEdit ? 'Editar Módulo' : 'Crear Módulo'" 
    :modal="true" 
    ...
    <form @submit.prevent="handleSubmit" class="flex flex-col gap-5 mt-2">
      <div class="flex flex-col gap-2">
        <label for="name" class="font-semibold text-[var(--surface-900)] flex items-center gap-2">
          <i class="pi pi-box text-[var(--primary-color)]"></i> Nombre del Módulo
        </label>
        <InputText id="name" v-model.trim="form.name" required autofocus class="w-full" placeholder="Ej: Usuarios" />
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <InputText id="description" v-model.trim="form.description" class="w-full" placeholder="Breve descripción del módulo" />
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <InputText id="icon" v-model.trim="form.icon" class="w-full" placeholder="Ej: pi pi-cog" />
      </div>
...
watch(() => props.moduleData, (newModule) => {
  if (newModule) {
    isEdit.value = true;
    form.id = newModule.id;
    form.name = newModule.name || '';
    form.description = newModule.description || '';
    form.icon = newModule.icon || '';
    form.status = newModule.status ?? true;
  } else {
    isEdit.value = false;
    form.id = null;
    form.name = '';
