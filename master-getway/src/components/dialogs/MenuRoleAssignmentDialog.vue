<!-- components/dialogs/MenuRoleAssignmentDialog.vue -->
<template>
  <Dialog 
    :visible="visible" 
    :header="`Asignar Roles a Menú: ${menu?.nombre || ''}`" 
    :modal="true" 
...
const handleAssign = async () => {
  if (!selectedRoleId.value || !props.menu) return;
  loadingAssign.value = true;
  try {
    await menuService.assignMenuToRole(props.menu.id, selectedRoleId.value);
    msjShow('success', 'Éxito', 'Menú asignado correctamente al rol', 3000);
    selectedRoleId.value = null;
  } catch (error) {
    console.error("Error al asignar menú al rol:", error);
    msjShow('error', 'Error', error.response?.data?.detail || 'No se pudo asignar el menú', 4000);
  } finally {
